...
const DeviceGallery = () => {
  ...
  const [filteredDevices, setFilteredDevices] = useState<DeviceBasicInfoProps[]>([]);

  const currentPage = Number(searchParams.get("page") || 1);
  const pageSize = 8; // Devices per page
  ...
  const weaponTypes = Array.from(new Set(devices.map((device) => device.weapon)));
